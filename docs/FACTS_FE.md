...
Items in the blob (facts, questions, rules, action conditions) are soft-deleted by setting `"discarded": true` rather than removing them. Reference values use a `discarded_at` timestamp. This keeps keys reserved, prevents reuse, and leaves company data untouched. The admin UI should show discarded items visually (greyed out / strikethrough) and allow un-discarding. The evaluation engine skips discarded items.
